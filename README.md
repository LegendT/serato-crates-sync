...
- **`~/Music/_Serato_/Subcrates/`** — one binary `.crate` file per crate
  / subcrate. The Subcrates folder is what `sync` writes to and what
  the `Subcrates.BACKUP.<timestamp>` rollback restores.
...
Serato versions; this tool does not touch it directly. On Serato DJ Pro
4.x the SQLite database is the source of truth.
...
This project is **not yet published to PyPI** — install from source:
